...
import org.finos.legend.depot.tracing.services.prometheus.DepotPrometheusMetricsHandler;
...
public class TestResourceApiTracing
{
    ...
    public void testMetricRegistrationForResource()
    {
        ...
        Assert.assertEquals(1, BaseResource.resourceMetricsRegistration.keySet().size());
        new TestBaseResource();
        Assert.assertEquals(1,prometheusMetrics.allSummaries.keySet().size());
        Assert.assertEquals(1, BaseResource.resourceMetricsRegistration.keySet().size());
    }
