...
public class ArtifactsPurgeServiceImpl implements ArtifactsPurgeService
{
    ...
    public String deleteSnapshotVersions(String groupId, String artifactId, List<String> versions)
    {
            ...
            versions.forEach(versionId ->
                    {
                        ...
                        {
                            nonCompliantVersionErrors.add(String.format("Version %s can't be deleted as it is not a snapshot version", versionId));
                        }
                        else if (defaultBranch.equals(versionId))
                        {
                            nonCompliantVersionErrors.add(String.format("Version %s can't be deleted as it is the project's default branch", defaultBranch));
                        }
                        else
                        {
        ...
        if (!nonCompliantVersionErrors.isEmpty())
        {
            return String.join("\n", nonCompliantVersionErrors) + "\nDeleted remaining versions";
        }
        return "Deleted all snapshot versions";
    }
